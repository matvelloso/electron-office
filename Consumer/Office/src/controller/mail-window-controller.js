...
const officeUrl = 'https://www.office.com/?auth=1'
const skypeUrl = 'https://web.skype.com/'
...
class MailWindowController {
    ...
    init() {
        ...
        this.skype = this.createWindow(skypeUrl);
        // Show window handler
    ...
    zoomOut() {
        this.mail.webContents.getZoomLevel((level) => {
            ...
            this.skype.webContents.setZoomLevel(level);
        });
    }
    ...
    zoomReset() {
        ...
        this.skype.webContents.setZoomLevel(0);
    }
    ...
    onWillRedirect(e, url) {
        ...
        if (url.startsWith('https://web.skype.com/')) {
           this.skype.setIcon(path.join(__dirname, '../../assets/skype_linux_black.png'))
        return;
        }
    ...
    onNewWindow(e, url) {
        ...
        if (url.indexOf('/mail') > 0 || url.startsWith("https://outlook.com/")) {
            ...
            return;
        }  else if (url.startsWith('https://1drv.ms')  || url.startsWith('https://www.office.com/') ) {
            ...
            this.onedrive.setIcon(path.join(__dirname, '../../assets/office_linux_black.png'))   
            return;
        } else if (url.startsWith('https://onedrive.live.com/') ) {
            if (this.activeWindow == this.onedrive) {
                e.preventDefault();
                this.onedrive.loadURL(url);                
                return;
            ...
            return;
        } else if (url.startsWith('https://web.skype.com/') ) {
            if (this.activeWindow == this.skype) {
                e.preventDefault();
                this.skype.loadURL(url);                
                return;
            }
            e.preventDefault()
            console.log('show skype');
            this.setActiveWindow(this.skype);
            this.skype.loadURL(url);
